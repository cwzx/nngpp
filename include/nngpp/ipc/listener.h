#ifndef NNGPP_IPC_LISTENER_H
#define NNGPP_IPC_LISTENER_H
...
struct listener : listener_view {
	...
	explicit listener( nng_ipc_listener* d ) noexcept : listener_view(d) {}
	...
	listener& operator=( listener&& rhs ) noexcept {
		if( this != &rhs ) {
			if( d != nullptr ) nng_ipc_listener_free(d);
			d = rhs.d;
			rhs.d = nullptr;
		}
		return *this;
	...
	nng_ipc_listener* release() noexcept {
		auto out = d;
		d = nullptr;
		return out;
	}
...
inline listener make_listener() {
	nng_ipc_listener* d;
	int r = nng_ipc_listener_alloc( &d );
	if( r != 0 ) {
		throw exception(r,"nng_ipc_listener_alloc");
	}
	return listener(d);
}
